...
<form onsubmit={handleSubmit} class="flex flex-col gap-3 p-3 rounded border border-blue-800 bg-gray-900/50">
  <TaskFormFields
    bind:title
    bind:description
    bind:level
  ...
  <div class="flex gap-2 justify-end pt-1">
    ...
    <button
      type="submit"
      disabled={!title.trim()}
      class="bg-blue-700 hover:bg-blue-600 disabled:opacity-40 disabled:cursor-not-allowed text-sm text-white px-3 py-1 rounded"
    >
      Сохранить
    </button>
  </div>
